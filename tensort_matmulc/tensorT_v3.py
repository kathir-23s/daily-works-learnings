...
#region init and utilities
    def __init__(self, data, _op=None, _parent=(), req_grad=False): #, req_grad=False):
...
# OPERATIONS
    ...
    def __add__(self, other):
        ...
        requires_grad = self.req_grad or (other.req_grad if isinstance(other, TensorT) else False)
        out = TensorT(result, _op='add', _parent=(self, other), req_grad=requires_grad)
    ...
    def __mul__(self, other):
        result = self._elementwise_op(other, lambda x, y: x * y)
        requires_grad = self.req_grad or (other.req_grad if isinstance(other, TensorT) else False)
        out = TensorT(result, _op='mul', _parent=(self, other), req_grad=requires_grad)
    ...
    def __sub__(self, other):
        result = self._elementwise_op(other, lambda x, y: x - y)
        requires_grad = self.req_grad or (other.req_grad if isinstance(other, TensorT) else False)
        out = TensorT(result, _op='sub', _parent=(self, other), req_grad=requires_grad)
    ...
    def __truediv__(self, other):
        result = self._elementwise_op(other, lambda x, y: x / y)
        requires_grad = self.req_grad or (other.req_grad if isinstance(other, TensorT) else False)
        out = TensorT(result, _op='div', _parent=(self, other), req_grad=requires_grad)
    ...
    def __neg__(self):
        return TensorT(self._apply_unary(self.data, lambda x: -x), req_grad=self.req_grad)
    
    def __pow__(self, other):
        return TensorT(self._apply_unary(self.data, lambda x : x**other), req_grad=self.req_grad)
    
    def tlog(self):
        """Element-wise natural logarithm."""
        return TensorT(self._apply_unary(self.data, math.log), req_grad=self.req_grad)

    def texp(self):
        """Element-wise exponential."""
        return TensorT(self._apply_unary(self.data, math.exp), req_grad=self.req_grad)
...
# DEFINING RANDOM TENSORS
    @classmethod
    def unit_tensor(cls, unit: float, shape, req_grad = False):
        ...
        return cls(build(shape), req_grad=req_grad)
    
    @classmethod
    def const_tensor(cls, unit: float, shape, req_grad = False):
        """Create a tensor filled with ones or zeros."""
        unit = float(unit)  # ensure float type
        def build(s):
            if len(s) == 1:
...
# MATRIX OPERATIONS
    def tmatmul_py_naive(self, other):
            ...
            requires_grad = self.req_grad or other.req_grad
            out = TensorT(result, _op='matmul', _parent=(self, other), req_grad=requires_grad)
            def backward_fn(grad_op):
                if not out.req_grad:
                    return None, None
    ...
    def tmatmul_py_fast(self, other):
        ...
        for i in range_m:
            ...
            for j in range_p:
                ...
                Ci[j] = s
        requires_grad = self.req_grad or other.req_grad 
        out = TensorT(C, _op='matmul', _parent=(self, other), req_grad=requires_grad)
    ...
    def tmatmul_c(self, other):
            ...
            requires_grad = self.req_grad or other.req_grad
            out = TensorT(result_ll, _op='matmul', _parent=(self, other), req_grad=requires_grad)
...
#region formatting and backward fns
    def ttranspose(self):
        ...
        return TensorT(tranposed_tensor, req_grad=self.req_grad)
    ...
    def treshape(self, new_shape: tuple):
        ...
        return TensorT(reshaped_tensor, req_grad=self.req_grad)
    ...
    def tsum_axis(self, axis=1, keepdims=True):
        ...
        if axis == 0:
            ...
            if keepdims:
                ...
                result = [sum(row) for row in self.data]
            return TensorT(result, req_grad=self.req_grad)
